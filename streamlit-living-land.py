...
st.caption('x-axis: Year | y-axis: Index(%)')

st.write('Above is a graph plotting the average index percentage of biodiversity per region. Note that all regions are on a steady decline, particularly Latin America which has a sharper decline than all other regions. One possible cause of this could be deforestation related to farming. See the below graph.')
...
st.write('The Global area of farmland is in its own graph as the area was so extreme that it flattened the line graph of the individual regions. Note the heavy drop around the 1990s, around the same time that four of the five regions had an increase in farmland usage.')
